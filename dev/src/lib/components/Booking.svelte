<script lang="ts">
	...
	const eventId = $derived($appStore.bookingState?.eventId);
	const eventTitle = $derived($appStore.bookingState?.eventTitle);
	const venueName = $derived($appStore.bookingState?.venueName);
	const eventBrandId = $derived($appStore.bookingState?.eventBrandId || '');
	const eventSchema = $derived($appStore.bookingState?.eventSchema || '');
	const eventSchemaPrice = $derived($appStore.bookingState?.eventSchemaPrice || 0);
	...
	const selectedDrinksDetails = $derived(
		Object.entries(selectedBrands).map(([brandId, quantity]) => {
			...
			const amount = quantity * eventSchemaPrice;
			return {
				brandId,
...
{#if !eventId || !eventTitle || !venueName}
	<Card>
		<CardContent class="p-4">
			<p class="text-muted-foreground">No booking in progress</p>
			<Button onclick={() => dispatch('navigate', { view: 'home' })}>Go Home</Button>
		</CardContent>
	</Card>
{:else}
	<div class="space-y-8">
		<div class="text-center space-y-4 pt-6">
			...
			<p class="text-muted-foreground">{eventTitle} at {venueName}</p>
		</div>
									...
									<div class="flex justify-between items-center">
										<div class="flex items-center gap-4">
											...
											<div>
												...
												{#if eventSchemaPrice}
													<p class="text-sm text-muted-foreground">{constants.CURRENCY_SYMBOL}{eventSchemaPrice}</p>
												{/if}
											</div>
										</div>
				...
				{:else if currentStep === 4}
					<div class="space-y-6">
						...
						<Card>
							...
							<CardContent class="space-y-3">
								<p class="text-sm">Event: {eventTitle}</p>
								<p class="text-sm">Venue: {venueName}</p>
